...
#---------------------------------------------------------------------------------
all: lib/libStardust.a

dist-bin: all
	@tar --exclude=*~ -cjf libStardust-$(VERSION).tar.bz2 include lib

dist-src:
	@tar --exclude=*~ -cjf libStardust-src-$(VERSION).tar.bz2 include source Makefile

dist: dist-src dist-bin

install: dist-bin
	mkdir -p $(DESTDIR)$(DEVKITPRO)/wut/usr
	bzip2 -cd libStardust-$(VERSION).tar.bz2 | tar -xf - -C $(DESTDIR)$(DEVKITPRO)/wut/usr
...
lib/libStardust.a :$(SOURCES) $(INCLUDES) | lib release
